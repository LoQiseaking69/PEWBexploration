...
using statistical analysis, hypothesis testing, and quantitative metrics.
"""
...
class ValidationData:
    ...
    def _compute_derived_quantities(self):
        ...
        # Theoretical predictions (from model)
        df['E_3dt_predicted'] = df['epsilon_squared'] * 3.27e14  # Scaling from data
        df['dt_predicted'] = 1.0 + 0.995 * df['epsilon_squared'] * 1e16  # Empirical fit
...
class StatisticalValidator:
    ...
    def test_claim_1_positive_energy(self) -> Dict:
        ...
        Null Hypothesis H0: E_total(epsilon) <= E_total(0) for all epsilon > 0
        Alternative H1: E_total(epsilon) > E_total(0) for some epsilon > 0
        """
        df = self.data.df
    ...
    def test_claim_2_epsilon_scaling(self) -> Dict:
        """
        CLAIM 2: Metric modifications scale as epsilon^2 (quantum perturbation theory).
        
        Null Hypothesis H0: dt_eff - 1 does NOT scale as epsilon^2
        Alternative H1: dt_eff - 1 = k * epsilon^2 + O(epsilon^4)
        """
        df = self.data.df
    ...
    def test_claim_3_velocity_reduction(self) -> Dict:
        """
        CLAIM 3: 3D time coupling reduces effective velocity (drag effect).
        
        Null Hypothesis H0: v_eff is independent of epsilon
        Alternative H1: v_eff decreases as epsilon increases
        """
        df = self.data.df
    ...
    def test_claim_4_no_horizon(self) -> Dict:
        ...
        Null Hypothesis H0: alpha^2 <= 0 for some epsilon
        Alternative H1: alpha^2 > 0 for all epsilon
        """
        df = self.data.df
    ...
    def test_claim_5_energy_finiteness(self) -> Dict:
        ...
        Null Hypothesis H0: Energy diverges or becomes unbounded
        Alternative H1: Energy is finite for all tested epsilon
        """
        df = self.data.df
        ...
        # Model: E = E0 + A*epsilon^2/(1 + B*epsilon^2)  (saturates at high epsilon)
    ...
    def test_claim_6_regime_transitions(self) -> Dict:
        ...
        Test for structural breaks in the data using change point detection.
        """
        df = self.data.df
        ...
        validation = {
            ...
            'validated': sum(matches) >= 1,  # At least one clear transition
...
class ModelCalibration:
    ...
    def calibrate_energy_model(self) -> Dict:
        ...
        Returns best-fit parameters with uncertainties.
        """
        df = self.data.df
        ...
        # Linear regression (no intercept for 3D term, but we include baseline)
        ...
        self.params['energy'] = {
            ...
            'E_3dt_coeff_J': E_3dt_coeff,  # Energy per epsilon^2
            'E_3dt_err': E_3dt_err,
            'r_squared': r_squared,
    ...
    def calibrate_metric_model(self) -> Dict:
        ...
        """
        # Use only quantum regime for perturbative calibration
        q = self.data.get_regime('quantum')
        
        X = q['epsilon_squared'].values
        y = q['dt_eff'].values - 1.0  # delta_dt
    ...
    def calibrate_velocity_model(self) -> Dict:
        ...
        # Fit power law reduction
        v0 = self.data.baseline['v_eff']
        delta_v = (v0 - df['v_eff']) / v0  # Fractional reduction
        
        # Fit: delta_v = c * epsilon^n
        log_eps = np.log10(df['epsilon'])
        log_dv = np.log10(delta_v + 1e-20)  # Avoid log(0)
        
        # Only fit where delta_v is significant
        mask = delta_v > 1e-10
        if mask.sum() > 5:
...
class PredictiveValidator:
    ...
    def extrapolation_test(self, epsilon_test: float = 1.0) -> Dict:
        ...
        """
        # Use calibrated model to predict at epsilon = 1.0 (way beyond tested range)
        if 'energy' in self.cal.params:
            p = self.cal.params['energy']
            E_pred = p['E_warp_J'] + p['E_3dt_coeff_J'] * epsilon_test**2
            
            # Physical sanity check: energy should not exceed solar mass equivalent
            M_sun = 1.989e30
            c = 2.998e8
            E_max_physical = M_sun * c**2  # One solar mass
...
def create_validation_plots(data: ValidationData, cal: ModelCalibration, 
    ...
    ax4.axvline(-8, color='k', linestyle='--', alpha=0.3)
    ax4.axvline(-4, color='k', linestyle='--', alpha=0.3)
    ...
    ax6.set_xlabel('ε')
    ax6.set_ylabel('α²')
    ax6.set_title('Horizon Safety: α² > 0')
    ax6.legend()
    ax6.grid(True, alpha=0.3)
    
    # [7] Power Spectrum (Energy growth rate)
    ax7 = plt.subplot(3, 3, 7)
    d_log_E = np.diff(np.log(df['E_total'])) / np.diff(df['log10_epsilon'])
    ax7.plot(df['log10_epsilon'].iloc[1:], d_log_E, 'ro-', markersize=3)
    ax7.set_xlabel('log₁₀(ε)')
    ax7.set_ylabel('d(log E)/d(log ε)')
    ax7.set_title('Energy Growth Rate')
    ax7.grid(True, alpha=0.3)
    
    # [8] 3D Time Contribution Fraction
    ax8 = plt.subplot(3, 3, 8)
    if 'energy' in cal.params:
        p = cal.params['energy']
        E_3dt = p['E_3dt_coeff_J'] * df['epsilon_squared']
        fraction = E_3dt / df['E_total'] * 100
        ax8.semilogx(df['epsilon'], fraction, 'go-', markersize=3)
        ax8.set_xlabel('ε')
        ax8.set_ylabel('3D Time Contribution [%]')
        ax8.set_title('Relative Energy Contribution')
    ...
    # Create text summary
    textstr = 'VALIDATION SCORECARD\n' + '='*25 + '\n\n'
    
    # This will be populated after validation runs
    textstr += 'Run validate_all() to populate'
    
    ax9.text(0.1, 0.9, textstr, transform=ax9.transAxes, fontsize=10,
...
def main():
    ...
    print("-" * 50)
    create_validation_plots(data, calibration)
